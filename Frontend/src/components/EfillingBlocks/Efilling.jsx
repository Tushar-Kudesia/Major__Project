...
const api = axios.create({
  baseURL: 'http://localhost:5000/api/cases',
  headers: {
...
const Efilling = () => {
  ...
  // <-- Updated handleCaseSubmit with validation-safe payload -->
  const handleCaseSubmit = useCallback(
    async (data) => {
      try {
        const payload = {
          litigant: {
            name: formData.litigant?.name || "",
            mobileNumber: formData.litigant?.mobileNumber || "",
            aadharNumber: formData.litigant?.aadharNumber || "",
            address: formData.litigant?.address || "",
            state: formData.litigant?.state || "",
            district: formData.litigant?.district || "",
          },
          case: {
            courtType: data.courtType || "",
            caseType: data.caseType || "",
            causeOfAction: data.causeOfAction || "",
            dateOfAction: new Date(data.dateOfAction).toISOString(),
        ...
        if (response.data?.message === "Case filed successfully") {
          setFormData((prev) => ({
            ...prev,
            case: {
              ...payload.case,
              dateOfAction: new Date(data.dateOfAction),
            },
          }));
          setCurrentStep("complete");
          toast.success("Case filed successfully!");
        } else {
          toast.error(response.data?.message || "Failed to file case");
        }
      } catch (error) {
        if (error.response?.data?.errors) {
          error.response.data.errors.forEach((err) => {
            toast.error(`${err.param || err.field}: ${err.msg || err.message}`);
          });
        } else {
